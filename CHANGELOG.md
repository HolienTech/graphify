...
- Feature: the graph now records **which model produced its semantic pass**. A knowledge graph is a durable artifact other systems query as fact, but "which model asserted this edge?" was previously unanswerable: no artifact recorded a model at all, and `_merge_into` received a per-chunk `model` from the provider response and dropped it. `graph.json` and `.graphify_analysis.json` now carry a `models` list, `GRAPH_REPORT.md` gains a `- Semantic model:` line, and `cost.json` records a per-run `models` alongside its per-run tokens. Where the provider reports what it actually served (the OpenAI-compat/Anthropic/Azure response `model`, and Claude Code's `modelUsage` envelope) that value is preferred and marked `reported`; where it does not (Bedrock's `converse`) the requested id is recorded and marked `requested`, so a silent fallback is never passed off as confirmed. A run that used more than one model — chunk-level fallback, adaptive-retry halves, a resumed or incremental run — records the whole **set**, since a single label on a mixed run is a lie. Attribution is derived only from the provider response or resolved config, never by asking a model what it is. Backwards compatible: an old `graph.json` with no `models` field loads and reads as `unknown`, never as a default model id, and an AST-only graph omits the field entirely. Fail-open throughout — attribution never breaks a build.

- Fix: `graphify prs --triage` reports the model that **answered** rather than only the one requested. It printed the requested model before the stream, which is indistinguishable from a silent fallback; it now labels that line `requested:` and prints an `answered by:` line afterwards, sourced from the provider's own response.
