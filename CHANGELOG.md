...
- **Feat: new `ollama-cloud` backend for semantic extraction.** Ollama Cloud's hosted, OpenAI-compatible inference, defaulting to Google's open-weight `gemma4:cloud` (multimodal, 256k context on the `31b` tag). Set `OLLAMA_CLOUD_API_KEY` (or Ollama's own `OLLAMA_API_KEY`) and pass `--backend ollama-cloud`; usage is covered by your Ollama plan's allowance rather than metered per token, so `graphify cost` reports $0. Kept as a backend distinct from `ollama` rather than an `OLLAMA_BASE_URL` override so it correctly opts out of the local-Ollama tuning — the `num_ctx` auto-derive sized for one local GPU, the serialized-call fallback, the zero-retry cap, the `GRAPHIFY_OLLAMA_VISION` opt-in, and the non-loopback corpus-egress warning — none of which apply to an endpoint you deliberately chose. Note: Ollama Cloud does **not** serve Gemini (closed weights); `gemma4` is the Google model available there.
- **Change: Gemini is no longer first in backend auto-detection.** It held that slot because of its free tier; now that it is metered like every other paid backend, a stray `GEMINI_API_KEY` outranking a backend the user actually configured is a surprise bill rather than a convenience. New `detect_backend()` order: `ollama-cloud` → `kimi` → `claude` → `openai` → `deepseek` → `gemini` → `azure` → `bedrock` → `ollama`. Gemini is still auto-detected when it is the only key set, and `--backend gemini` is unaffected. `ollama-cloud` leads because plan-covered inference should never lose to a metered backend. As a safeguard, `ollama-cloud` is *not* auto-selected when `OLLAMA_BASE_URL` points at loopback: that user asked for extraction to stay on their machine, and silently shipping the corpus to a hosted endpoint on a leftover `OLLAMA_API_KEY` is the F-002 egress failure mode in reverse — `--backend ollama-cloud` still selects it explicitly.
- Fix: backend-detection tests are hermetic again. `tests/test_llm_backends.py` and `tests/test_ollama.py` each cleared an ad-hoc subset of the environment, so a key they didn't think to clear (`GEMINI_API_KEY` in practice, plus model overrides like `GRAPHIFY_GEMINI_MODEL`) leaked in from the developer's shell — the suite failed locally while passing in clean CI. Both now clear the full set `detect_backend()` consults.
